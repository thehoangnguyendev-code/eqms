...
import type { BusinessUnitItem } from "../types";
import { MOCK_BUSINESS_UNITS, MOCK_DEPARTMENTS } from "../mockData";
...
>((_, ref) => {
  ...
  const [selectedItem, setSelectedItem] = useState<BusinessUnitItem | null>(null);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [mockData, setMockData] = useState<BusinessUnitItem[]>(MOCK_BUSINESS_UNITS);
    ...
    filterFn: (item, query) => {
      ...
      if (modifiedFromDate) {
        ...
        if (parts) {
          ...
          matchesModifiedFrom = new Date(item.modifiedDate) >= from;
        }
      }
      if (modifiedToDate) {
        ...
        if (parts) {
          ...
          matchesModifiedTo = new Date(item.modifiedDate) <= to;
        }
      }
  ...
  // Count departments per business unit for display
  const departmentCountByUnit = useMemo(() => {
    const counts: Record<string, number> = {};
    for (const dept of MOCK_DEPARTMENTS) {
      const unit = mockData.find((u) => u.name === dept.businessUnit);
      if (unit) counts[unit.id] = (counts[unit.id] ?? 0) + 1;
    }
    return counts;
  }, [mockData]);

  const handleEdit = (item: BusinessUnitItem) => {
    setSelectedItem(item);
    setShowEditModal(true);
    closeDropdown();
  };

  const handleDelete = (item: BusinessUnitItem) => {
    setSelectedItem(item);
    setShowDeleteModal(true);
    closeDropdown();
  };

  const handleToggleStatus = (item: BusinessUnitItem) => {
    setMockData((prev) =>
                ...
                {[
                  ...
                  return (
                    <th
                      ...
                      className={cn(
                        "py-2.5 px-2 sm:py-3.5 sm:px-4 text-left text-[10px] sm:text-xs font-bold text-slate-500 uppercase tracking-wider whitespace-nowrap transition-colors group",
              ...
              {sortedItems.length > 0 ? (
                paginatedItems.map((item, index) => (
                  <tr key={item.id} className="hover:bg-slate-50/80 transition-colors group">
                    ...
                    <td className="py-2 px-2 sm:py-3.5 sm:px-4 text-xs sm:text-sm text-slate-600 max-w-xs truncate">
                      {item.description || "-"}
                    </td>
                    <td className="py-2 px-2 sm:py-3.5 sm:px-4 text-xs sm:text-sm whitespace-nowrap">
                      {item.isActive ? (
            ...
            setMockData((prev) =>
              prev.map((i) =>
                i.id === selectedItem.id
                  ? { ...i, ...data, modifiedDate: new Date().toISOString().split("T")[0] }
                  : i,
              ),
            );
          } else {
            const newItem: BusinessUnitItem = {
              id: String(Date.now()),
              ...data,
          ...
          <div className="space-y-3">
            ...
            <div className="text-xs bg-amber-50 border border-amber-200 rounded-lg p-3">
              <p className="text-amber-800">
                ...
                <span className="font-semibold">Warning:</span> Departments assigned to this
                business unit will lose their association. This action cannot be undone.
              </p>
            </div>
          </div>
        }
...
interface BusinessUnitModalProps {
  ...
  isEdit: boolean;
  onSave: (data: Omit<BusinessUnitItem, "id" | "createdDate" | "modifiedDate">) => void;
}
...
const BusinessUnitModal: React.FC<BusinessUnitModalProps> = ({
  ...
  return (
    <FormModal
      ...
      title={`${isEdit ? "Edit" : "Add New"} Business Unit`}
      isLoading={isSaving}
      size="xl"
    >
      <div className="space-y-4">
        <div>
          ...
          <input
            ...
            placeholder="e.g., Operation Unit"
            required
            disabled={isSaving}
          />
        </div>
