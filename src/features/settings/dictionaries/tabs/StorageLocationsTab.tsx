...
import { MOCK_STORAGE_LOCATIONS } from "../mockData";
...
export const StorageLocationsTab = React.forwardRef<{ openAddModal: () => void }, {}>((_, ref) => {
  ...
  const [mockData, setMockData] = useState<StorageLocationItem[]>(MOCK_STORAGE_LOCATIONS);
  ...
  // Use the shared table filter hook
  const {
    searchQuery,
    setSearchQuery,
    currentPage,
  ...
  const handleToggleStatus = (item: StorageLocationItem) => {
    setMockData((prev) =>
      ...
      <StorageLocationModal
        ...
        onSave={(data) => {
          if (showEditModal && selectedItem) {
            ...
            const newItem: StorageLocationItem = {
...
const StorageLocationModal: React.FC<StorageLocationModalProps> = ({
  ...
  return (
    <FormModal
      ...
      title={`${isEdit ? "Edit" : "Add New"} Storage Location`}
      isLoading={isSaving}
      size="xl"
    >
