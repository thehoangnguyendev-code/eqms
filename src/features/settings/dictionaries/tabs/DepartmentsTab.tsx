...
>((_, ref) => {
  ...
  const handleToggleStatus = (item: DepartmentItem) => {
    setMockData((prev) =>
              ...
              {sortedItems.length > 0 ? (
                paginatedItemsFromSorted.map((item, index) => (
                  ...
                  >
                    ...
                    <td className="py-2 px-2 sm:py-3.5 sm:px-4 text-xs sm:text-sm whitespace-nowrap">
                      <Badge
                        color={item.businessUnit === "Operation Unit" ? "purple" : "cyan"}
                        size="sm"
                      >
                        {item.businessUnit}
                      </Badge>
...
const DepartmentModal: React.FC<DepartmentModalProps> = ({
  ...
  return (
    <FormModal
      ...
      title={`${isEdit ? "Edit" : "Add New"} Department`}
      isLoading={isSaving}
      size="xl"
    >
