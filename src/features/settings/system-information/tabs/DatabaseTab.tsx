    ...
    <div className="p-5 space-y-4">
      ...
      <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
        <div className="flex items-center gap-3 px-5 py-3.5 bg-slate-50/70 border-b border-slate-200">
          ...
          <div>
            <h3 className="text-sm font-semibold text-slate-900">Database Configuration</h3>
            <p className="text-xs text-slate-500 mt-0.5">Type, version, host & connection details</p>
          </div>
        </div>
        <div className="p-5">
      ...
      {data.connectionPool && (
        <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
          <div className="flex items-center gap-3 px-5 py-3.5 bg-slate-50/70 border-b border-slate-200">
            ...
            <div>
              <h3 className="text-sm font-semibold text-slate-900">Connection Pool</h3>
              <p className="text-xs text-slate-500 mt-0.5">Active, idle & waiting connections</p>
            </div>
          </div>
          <div className="p-5 space-y-4">
      ...
      {data.tableStats && data.tableStats.length > 0 && (
        <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
          <div className="flex items-center gap-3 px-5 py-3.5 bg-slate-50/70 border-b border-slate-200">
            ...
            <div>
              <h3 className="text-sm font-semibold text-slate-900">Table Statistics</h3>
              <p className="text-xs text-slate-500 mt-0.5">{data.tableStats.length} tables tracked</p>
            </div>
          </div>
              ...
              <tbody className="divide-y divide-slate-200 bg-white">
                {data.tableStats.map((table) => (
                  <tr key={table.name} className="hover:bg-slate-50/80 transition-colors">
                    <td className="py-2 px-2 sm:py-3.5 sm:px-4 text-xs sm:text-sm font-semibold text-slate-900 ">{table.name}</td>
      ...
      <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
        <div className="flex items-center gap-3 px-5 py-3.5 bg-slate-50/70 border-b border-slate-200">
          ...
          <div>
            <h3 className="text-sm font-semibold text-slate-900">Backup Information</h3>
            <p className="text-xs text-slate-500 mt-0.5">Most recent database backup timestamp</p>
          </div>
        </div>
        <div className="p-5">
          <div>
