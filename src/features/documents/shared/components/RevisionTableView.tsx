...
interface RevisionTableViewProps<T extends Record<string, any>> {
  // Data
  revisions: T[];
  columns: TableColumn[];
 ...
 */
export const RevisionTableView = React.forwardRef<HTMLDivElement, RevisionTableViewProps>(
  (
    {
      revisions,
      columns,
    ...
    const totalPages = Math.ceil(revisions.length / itemsPerPage);
    ...
    if (paginatedRevisions.length === 0) {
      return (
        <div className="rounded-xl border border-slate-200 bg-white p-8 text-center">
          <TableEmptyState title={emptyStateTitle} message={emptyStateMessage} />
        </div>
      );
    }
            ...
            <table className="w-full min-w-max border-separate border-spacing-0 text-left">
              ...
              <thead>
                <tr>
                  ...
                  <th className="sticky top-0 z-20 bg-slate-50 py-2.5 px-2 md:py-3.5 md:px-4 text-[10px] md:text-[11px] font-bold text-slate-500 uppercase tracking-wider border-b-2 border-slate-200 whitespace-nowrap w-9" />
              ...
              <tbody className="bg-white">
                {paginatedRevisions.map((item, index) => {
                  ...
                  const tdClass =
                    "py-2.5 px-2 md:py-3 md:px-4 text-xs md:text-sm text-slate-700 border-b border-slate-200 whitespace-nowrap";
                  ...
                  return (
                    <React.Fragment key={itemId}>
                      ...
                      <tr className="hover:bg-slate-50/80 transition-colors group">
                        ...
                        <td
                          className="py-2.5 px-2 md:py-3 md:px-3 border-b border-slate-200 whitespace-nowrap"
                          onClick={(e) => {
                            e.stopPropagation();
...
RevisionTableView.displayName = "RevisionTableView";
