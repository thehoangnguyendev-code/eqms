...
const mockAuditData: AuditEntry[] = [
  {
    ...
    actionType: "approve",
    reason:
      "Document meets all GMP requirements and is ready for implementation",
...
export const AuditTrailTab: React.FC = () => {
  ...
  const filteredData = mockAuditData.filter((entry) => {
    const matchesAction =
      selectedAction === "all" || entry.actionType === selectedAction;
    const matchesSearch =
      entry.user.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
      entry.action.toLowerCase().includes(searchQuery.toLowerCase());
    const matchesUser =
      selectedUser === "all" || entry.user.name === selectedUser;
    const matchesDepartment =
      selectedDepartment === "all" ||
      entry.user.department === selectedDepartment;
