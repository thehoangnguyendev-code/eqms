...
export const AuditTrailTab: React.FC = () => {
  ...
  const filteredData = mockAuditData.filter((entry) => {
    ...
    return matchesAction && matchesSearch && matchesUser && matchesDepartment && matchesDateFrom && matchesDateTo;
  });
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-12 gap-4 items-end">
          ...
          <div className="xl:col-span-4 w-full">
            <Select
              ...
              onChange={(value) => { setSelectedAction(value as string); setCurrentPage(1); }}
          ...
          <div className="xl:col-span-4 w-full">
            <Select
              label="User"
              value={selectedUser}
              onChange={(value) => { setSelectedUser(value as string); setCurrentPage(1); }}
              options={userOptions}
          ...
          <div className="xl:col-span-4 w-full">
            <Select
              ...
              onChange={(value) => { setSelectedDepartment(value as string); setCurrentPage(1); }}
          ...
          <div className="md:col-span-2 xl:col-span-4 w-full">
            <DateRangePicker
              ...
              startDate={dateFrom}
              endDate={dateTo}
              onStartDateChange={(dateStr) => { setDateFrom(dateStr); setCurrentPage(1); }}
              onEndDateChange={(dateStr) => { setDateTo(dateStr); setCurrentPage(1); }}
        ...
        <div className="overflow-x-auto flex-1">
          <table className="w-full">
            <thead className="bg-slate-50 border-b border-slate-200 sticky top-0 z-30">
              <tr>
                <th className="px-2 py-2.5 sm:px-4 sm:py-3.5 text-center text-[10px] sm:text-xs font-bold text-slate-500 uppercase tracking-wider whitespace-nowrap">No.</th>
                <th className="px-2 py-2.5 sm:px-4 sm:py-3.5 text-left text-[10px] sm:text-xs font-bold text-slate-500 uppercase tracking-wider whitespace-nowrap">Timestamp</th>
                <th className="px-2 py-2.5 sm:px-4 sm:py-3.5 text-left text-[10px] sm:text-xs font-bold text-slate-500 uppercase tracking-wider whitespace-nowrap">User</th>
                <th className="px-2 py-2.5 sm:px-4 sm:py-3.5 text-left text-[10px] sm:text-xs font-bold text-slate-500 uppercase tracking-wider whitespace-nowrap">Action</th>
                <th className="px-2 py-2.5 sm:px-4 sm:py-3.5 text-left text-[10px] sm:text-xs font-bold text-slate-500 uppercase tracking-wider whitespace-nowrap">Changes</th>
                <th className="px-2 py-2.5 sm:px-4 sm:py-3.5 text-left text-[10px] sm:text-xs font-bold text-slate-500 uppercase tracking-wider whitespace-nowrap">Reason</th>
                <th className="px-2 py-2.5 sm:px-4 sm:py-3.5 text-left text-[10px] sm:text-xs font-bold text-slate-500 uppercase tracking-wider whitespace-nowrap">IP / Device</th>
              </tr>
            </thead>
              ...
              {paginatedData.length === 0 ? (
                ...
                paginatedData.map((entry, index) => (
                  <tr key={entry.id} className="hover:bg-slate-50/80 transition-colors">
                    ...
                    <td className="px-2 py-2 sm:px-4 sm:py-4 whitespace-nowrap">
                      <div>
                        <div className="text-xs sm:text-sm font-medium text-slate-900">{entry.user.name}</div>
                        <div className="text-[10px] sm:text-xs text-slate-600">{entry.user.role}</div>
                        <div className="text-[10px] sm:text-xs text-slate-500">{entry.user.department}</div>
                      </div>
                    </td>
                    <td className="px-2 py-2 sm:px-4 sm:py-4 whitespace-nowrap">
                      <span className={cn(
                    ...
                    <td className="px-2 py-2 sm:px-4 sm:py-4 whitespace-nowrap">
                      <div className="flex items-start gap-1.5 sm:gap-2">
                        ...
                        <div>
                          <div className="text-[10px] sm:text-xs text-slate-700">{entry.ipAddress}</div>
              ...
              <div className="space-y-4">
                {selectedEntry.changes.map((change, index) => (
                  <div key={index} className="border border-slate-200 rounded-lg p-4">
                    <div className="text-xs sm:text-sm font-semibold text-slate-900 mb-3">{change.field}</div>
